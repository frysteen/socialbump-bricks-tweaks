...
class SBBT_Acf_Source {
	...
	public static function notice() {
		...
		$text = sprintf(
			esc_html__( 'ACF is running from the copy bundled with Advanced Themer, version %s, rather than from a plugin of its own.', 'sb-bricks-tweaks' ),
			esc_html( $running )
		);

		if ( $own !== '' ) {
			$text .= ' ' . sprintf(
				esc_html__( '%s is installed but switched off. Activate it under Plugins to use that one instead.', 'sb-bricks-tweaks' ),
				esc_html( $own )
			);
		}
