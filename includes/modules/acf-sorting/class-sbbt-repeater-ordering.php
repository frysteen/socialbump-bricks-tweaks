...
class SBBT_Repeater_Ordering {
	...
	public static function init() {
		...
		foreach ( (array) apply_filters( 'sbbt/repeater_ordering/elements', self::ELEMENTS ) as $name ) {
	 ...
	 * is US English. Change that with the sbbt/repeater_ordering/day_first filter.
	 */
	private static function timestamp( $value ) {
		...
		if ( preg_match( '#^(\d{1,2})[/.\-](\d{1,2})[/.\-](\d{4}|\d{2})(?:[\sT]+(.+))?$#', $value, $m ) ) {
			$day_first = (bool) apply_filters( 'sbbt/repeater_ordering/day_first', get_locale() !== 'en_US' );
	...
	public static function snippet_notice() {
		...
		esc_html_e( 'ACF Repeater Ordering is paused on this site because the old SocialBUMP repeater ordering snippet is still switched on. Turn that snippet off in WP CodeBox and the plugin takes over. Loops already set up keep their settings.', 'sb-bricks-tweaks' );
		echo '</p></div>';
	}
}