...
class SBBT_Gutenberg_Styles {
	...
	public static function enqueue( array $names, $content = '' ) {
		...
		wp_register_style( 'sbbt-block-common', includes_url( 'css/dist/block-library/common.min.css' ), [], $version );
		wp_enqueue_style( 'sbbt-block-common' );
		...
		if ( $css !== '' ) {
			wp_register_style( 'sbbt-block-layout', false, [], $version );
			wp_add_inline_style( 'sbbt-block-layout', $css );
			wp_enqueue_style( 'sbbt-block-layout' );
		}
	}
	...
	public static function default_gap() {
		...
		return (string) apply_filters( 'sbbt/gutenberg_styles/default_gallery_gap', self::gap_value( (string) $value ) );
	}
	...
	public static function editor_styles() {
		...
		if ( file_exists( $variables ) ) {
			wp_enqueue_style( 'sbbt-bricks-variables', trailingslashit( $uploads['baseurl'] ) . 'bricks/css/global-variables.min.css', [], (string) filemtime( $variables ) );
		}

		wp_register_style( 'sbbt-gallery-gap', false, [], null );
		wp_add_inline_style( 'sbbt-gallery-gap', $css );
		wp_enqueue_style( 'sbbt-gallery-gap' );
	}
