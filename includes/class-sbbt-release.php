...
class SBBT_Release {
	...
	const CHANGES_OPTION = 'sbbt_pending_changes';
	...
	private function is_published( $token, $release_id ) {
		$res = $this->github( 'GET', '/repos/' . SBBT_GITHUB_REPO . '/releases/' . (int) $release_id, $token );
	...
	private function abort( $message, $original = null, $zip = '', $token = '', $release_id = 0, $readme_original = null ) {
		...
		if ( $release_id && $token ) {
			$deleted = $this->github( 'DELETE', '/repos/' . SBBT_GITHUB_REPO . '/releases/' . (int) $release_id, $token );

			// Say so when the tidy up fails, rather than leaving a draft nobody knows about.
			if ( (int) $deleted['code'] !== 204 ) {
				$message .= ' ' . __( 'A draft release was left behind on GitHub and needs deleting by hand.', 'sb-bricks-tweaks' );
			}
		}
	...
	public function publish() {
		...
		if ( $this->asset_count( $token, $release_id ) < 1 ) {
			$this->abort( __( 'GitHub took the zip but did not attach it to the release.', 'sb-bricks-tweaks' ), $original, $zip, $token, $release_id, $readme_original );
		}
		 ...
		 */
		$live = $this->github_retry( 'PATCH', '/repos/' . SBBT_GITHUB_REPO . '/releases/' . $release_id, $token, [ 'draft' => false ] );
		...
		$this->github_retry( 'PATCH', '/repos/' . SBBT_GITHUB_REPO . '/releases/' . $release_id, $token, [ 'make_latest' => 'true' ] );
					...
					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
						...
						<?php if ( $changes ) : ?>
							<p class="sbbt-card__desc">
								...
								printf( esc_html( _n( 'Filled in from %s change noted since the last release. Edit it before publishing if you like.', 'Filled in from %s changes noted since the last release. Edit it before publishing if you like.', count( $changes ), 'sb-bricks-tweaks' ) ), esc_html( number_format_i18n( count( $changes ) ) ) );
								?>
							</p>
						<?php endif; ?>
						<div class="sbbt-release__actions">
